...
const FileList = () => {
	...
	useEffect(() => {
		const fetchFiles = async () => {
			try {
				...
				const response = await fetch("http://65.2.70.232/api/upload/getFiles", {
					method: 'GET',
					headers: {
				...
				if (response.ok) {
					...
					const data = await response.json();
					alert(`Failed to get Files: ${data.message}`);
				}
			} catch (error) {
				console.error('Error occurred while getting file:', error);
				alert('An error occurred while getting file. Please try again later.');
			}
		}
		fetchFiles();
	}, []);
	...
	const handleView = async (item) => {
		try {
			const jwtToken = await AsyncStorage.getItem('jwtToken');
			const response = await fetch("http://65.2.70.232/api/upload/objectUrl", {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${jwtToken}`,
			...
			if (response.ok) {
				...
				const data = await response.json();
				alert(`Failed to get file: ${data.message}`);
			}
		} catch (error) {
			console.error('Error occurred while downloading file:', error);
			alert('An error occurred while downloading file. Please try again later.');
		}
	}
	return (
		<View style={{ flex: 1 }}>
